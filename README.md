...
- **Rate limits on every auth and lookup route**, plus coupon checks — a short code space is otherwise walkable from a browser console. They bucket on the address the *proxy* reported, not the one the caller claimed: `X-Forwarded-For` is a list every proxy appends to, so its left-most entry is written by whoever is being limited. Reading it bought a fresh window per request and made all of these decorative. And they bucket per *shopper*: both Next servers forward the address they read to the API, because every call they make on somebody's behalf leaves from one container — so the API was counting the whole shop as a single client, and five sign-in codes a minute was five between every visitor at once rather than five for each of them.
