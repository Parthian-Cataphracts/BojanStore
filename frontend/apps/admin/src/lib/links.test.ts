/**
 * Every internal link in the panel has to point at a route that exists.
 *
 * The "افزودن کاربر" button on screen 145 linked to /settings/users/new for as
 * long as that screen has existed. There is no such route, and no endpoint to
 * build one against — the API only lists operators. Nothing catches that: it
 * type-checks, it lints, and it renders as a perfectly ordinary button. The
 * only way to find it is to click it, and the only way to keep it found is a
 * test that walks the links.
 */
