 ...
 *
 * The session cookie is `SameSite=Lax` here — `Strict` in the panel — which
 * already keeps it off a cross-site POST in any browser that honours it. That
 * was the whole defence, and it is one attribute away from nothing: a
 * relaxation made for some future integration, or a client that treats an
 * unknown value as `None`. Neither should be the difference between a
 * customer's session being safe and being spendable by any page they open.
 *
